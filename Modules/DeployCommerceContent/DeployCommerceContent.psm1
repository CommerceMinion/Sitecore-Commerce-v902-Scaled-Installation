...
Function Invoke-DeployCommerceContentTask {
    ...
    param(
		...
		[Parameter(Mandatory=$false)]
        [string]$CommerceAuthoringService,		
		[Parameter(Mandatory=$false)]
        [string]$CommerceIdService,		
		[Parameter(Mandatory=$false)]
        [string]$CommerceBizFxService
    ...
    try {       
        switch ($Name) {
            {($_ -match "CommerceOps")} {                    
				...
				$originalJson.AppSettings.SitecoreIdentityServerUrl = $originalJson.AppSettings.SitecoreIdentityServerUrl -replace "https://localhost", $CommerceIdService
				$originalJson.AppSettings.AllowedOrigins[0] = $originalJson.AppSettings.AllowedOrigins[0] -replace "https://localhost", $CommerceBizFxService
            ...
            'SitecoreIdentityServer' {
				...
				$originalJson.AppSettings.Clients[0].RedirectUris[2] = $originalJson.AppSettings.Clients[0].RedirectUris[2] -replace "https://localhost", $CommerceBizFxService
				$originalJson.AppSettings.Clients[0].RedirectUris[3] = $originalJson.AppSettings.Clients[0].RedirectUris[3] -replace "https://localhost", $CommerceBizFxService

				$originalJson.AppSettings.Clients[0].PostLogoutRedirectUris[2] = $originalJson.AppSettings.Clients[0].PostLogoutRedirectUris[2] -replace "https://localhost", $CommerceBizFxService
				$originalJson.AppSettings.Clients[0].PostLogoutRedirectUris[3] = $originalJson.AppSettings.Clients[0].PostLogoutRedirectUris[3] -replace "https://localhost", $CommerceBizFxService

				$originalJson.AppSettings.Clients[0].AllowedCorsOrigins[2] = $originalJson.AppSettings.Clients[0].AllowedCorsOrigins[2] -replace "https://localhost", $CommerceBizFxService
				$originalJson.AppSettings.Clients[0].AllowedCorsOrigins[3] = $originalJson.AppSettings.Clients[0].AllowedCorsOrigins[3] -replace "https://localhost", $CommerceBizFxService

				$originalJson | ConvertTo-Json -Depth 100 | set-content $appSettingsPath						
			}
			'SitecoreBizFx' {
				Write-Host
				...
				Write-Host "BizFx copy completed" -ForegroundColor Green ;	
					...
					Write-Host "Loading JSON for parameter replacement from $pathToJson" -ForegroundColor Yellow ;

				    $originalJson = Get-Content $pathToJson -Raw  | ConvertFrom-Json

					# Replacements
					$originalJson.EngineUri = $originalJson.EngineUri.replace("https://localhost", $CommerceAuthoringService)
					$originalJson.EngineUri = $originalJson.EngineUri.replace("5000", $CommerceAuthoringServicesPort)
					$originalJson.IdentityServerUri = $originalJson.IdentityServerUri.replace("https://localhost", $CommerceIdService)
					$originalJson.BizFxUri = $originalJson.BizFxUri.replace("https://localhost", $CommerceBizFxService)
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUGs9D48QONqoeYVO9KXRbKvMv
# CJegghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJT8Jls+j4juvaLMj6bAT3xm
# OviVMA0GCSqGSIb3DQEBAQUABIIBABP4DXm+2smz8CN1Z+Te1QE6hNzfUgVEo5vJ
# avtTnMwI94FSpHMTcHipv7y9l5FXDVwmjB62vMR50KwmEUQ/dJjK/XRcDDhRcUtO
# or+5d4sGtqw4osZss3sMByOfvCl+bPxB5rZTajb6ONA5+sreqjtwqLK7Ml1eWZR+
# O9aDVqBTTTtRimxRyYL0uR3QqpzmR8obepquilf6Xdu1jfOoFnU++TR6K+Tho/45
# FcbVYPyybhBIpALnqNH0tdtnQHe1AWk+Uc9tpGtrx1Gf9E/x9XddRECHgs4JIHCn
# V/ocyivxmWKiTwlwVsKTpUR/tmE/VuOCrfxONHp5caKFATyd/iShggILMIICBwYJ
...
# MTgwMzEyMDc0MjAyWjAjBgkqhkiG9w0BCQQxFgQUi1lLHBzWGc+Qg1hGimxwHike
# azQwDQYJKoZIhvcNAQEBBQAEggEAkDtRhv8TTZTEL2KYNLID0WlETLi3BrnxDSVW
# 8GmFimKvXuohpTueUb+6eDOtEI/Ec8dsOewYSWQ73EtlaH9jp8gwssnQqlJn3mmT
# Q7impcAcRuX2Eh74fFmQs5kS13QtL+Niy7oY9WHT+6LM4N1uBhwFRdqUCMRyxVet
# z24OWYCqkplGT3kAU+Aw71onuNfQGNpG4ZBolPsbJAPnkmhDkHMMC7f3gCpi0C2V
# lCSIJ7puUyBq6WIDshrfJP8hbGHnsiVPifm8x+KzgwA3pVbuGbmGujFKrpIcXd3w
# U6C57o+oRqiTqq0xQw4jEhybT9FSCFbQCJDOvIQQ3yV1XmoQpA==
