...
param(
	...
	[string]$SiteHostHeaderName = "xp902-cm",	
...
$params = @{
        Path = Resolve-Path '.\Configuration\Commerce\Master_SitecorePackagesCM.json'	
		SiteName = $SiteName
		SiteHostHeaderName = $SiteHostHeaderName 
		InstallDir = "C:\inetpub\wwwroot\xp902-cm"
        ...
        HabitatImagesModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore.Commerce.Habitat.Images-1.0.0.zip"	
        AdvImagesModuleFullPath = Resolve-Path -Path "$ResourceRoot\Adventure Works Images.zip"	
        CommerceConnectModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Connect Core 11.2.83.zip"	
		
		CommercexProfilesModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce ExperienceProfile Core *.zip"	
		CommercexAnalyticsModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce ExperienceAnalytics Core *.zip"	
		CommerceMAModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Marketing Automation Core *.zip"	
		CommerceMAForAutomationEngineModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Marketing Automation for AutomationEngine *.zip"

		CEConnectPackageFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore.Commerce.Engine.Connect.2.2.86.update"
        ...
        SXACommerceModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Experience Accelerator 1.2.205.zip"
		SXAStorefrontModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Experience Accelerator Storefront 1.2.205.zip"
        SXAStorefrontThemeModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Experience Accelerator Storefront Themes 1.2.205.zip"
		SXAStorefrontCatalogModuleFullPath = Resolve-Path -Path "$ResourceRoot\Sitecore Commerce Experience Accelerator Habitat Catalog 1.2.205.zip"
		MergeToolFullPath = Resolve-Path -Path "c:\deploy_xc\Microsoft.Web.XmlTransform.dll"
