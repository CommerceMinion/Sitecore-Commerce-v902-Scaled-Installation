...
$params = @{
        Path = Resolve-Path '.\Configuration\Commerce\Master_CommerceEngine.json'	
		SiteName = $SiteName
		SiteHostHeaderName = $SiteHostHeaderName 
		CertificateName = $SiteName
		CommerceServicesDbServer = $SqlServer
		CommerceServicesDbId = $SqlAdminUser
		CommerceServicesDbPw = $SqlAdminPassword
		...
		SolrUrl = "https://xc-xp1-solr/solr"
		SolrSchemas = ( Join-Path -Path $DEPLOYMENT_DIRECTORY -ChildPath "SolrSchemas" )
		SearchIndexPrefix = ""
		AzureSearchServiceName = ""
		AzureSearchAdminKey = ""
		AzureSearchQueryKey = ""
		CommerceEngineDacPac = Resolve-Path -Path "$ResourceRoot\Sitecore.Commerce.Engine.SDK.2.2.72\Sitecore.Commerce.Engine.DB.dacpac"	   
		...
		SitecoreCommerceEngineZipPath = Resolve-Path -Path "$ResourceRoot\Sitecore.Commerce.Engine.2.2.126.zip"		
		...
		CommerceIdService = $CommerceIdService
		CommerceBizFxService = $CommerceBizFxService
    }
